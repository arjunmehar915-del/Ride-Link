...
function writeAuth(v: Auth | null) {
  if (v) localStorage.setItem("ridelink:auth", JSON.stringify(v));
  else localStorage.removeItem("ridelink:auth");
}
...
export default function Account() {
  ...
  const saveKyc = () => {
    if (!auth) return;
    if (!license || !rc || !aadhaar) {
      toast.error("Upload Licence, RC and Aadhaar");
      return;
    }
    const updated: Auth = {
      ...auth,
  ...
  const kycDone = useMemo(
    () => !!(auth?.docs?.license && auth.docs.rc && auth.docs.aadhaar),
    [auth],
  );
    ...
    <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-2">
          <User2 className="h-7 w-7 text-primary" /> Account
        </h1>
        {auth && (
        ...
        <Card>
          ...
          <CardContent>
            {auth?.role === "rider" ? (
              <>
                <div className="text-sm text-muted-foreground">
                  Current status: {kycDone ? "Complete" : "Incomplete"}
                </div>
                <div className="mt-4 grid gap-4 sm:grid-cols-3">
                  ...
                  <div>
                    ...
                    {auth.docs?.aadhaar && (
                      <div className="mt-1 text-xs text-muted-foreground">
                        On file: {auth.docs.aadhaar}
                      </div>
                    )}
                  </div>
                </div>
                <div className="mt-4 flex gap-3">
                  <Button className="gap-2" onClick={saveKyc}>
              ...
              <Alert>
                ...
                <AlertDescription>
                  To offer rides, switch your role to Rider and complete KYC.
                  You can always ride as a passenger without documents.
                </AlertDescription>
              </Alert>
            )}
          </CardContent>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle className="text-xl flex items-center gap-2">
              <History className="h-5 w-5 text-primary" /> Rides & billing
            </CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground space-y-2">
            <div>
              View your past trips, download invoices, and manage refunds.
            </div>
        ...
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground space-y-2">
            <div>
              Communication: transactional emails and SMS for ride updates.
            </div>
            <div>Privacy: your phone and email are masked during rides.</div>
            <div>
              Location: live location is shared only with active trips you
              start.
            </div>
            <div className="pt-2">
              Need a change not listed here?{" "}
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2">
        <Card>
          ...
          <CardContent className="text-sm text-muted-foreground space-y-2">
            <div>
              Chat with our bot or email the team. Safety issues are
              prioritized.
            </div>
            <div className="flex flex-wrap gap-3">
              ...
              <Button asChild variant="outline" className="gap-2">
                <a href="mailto:support@ridelink.example">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-xl flex items-center gap-2">
              <ExternalLink className="h-5 w-5 text-primary" /> Links
            </CardTitle>
          </CardHeader>
          <CardContent className="text-sm text-muted-foreground space-y-2">
            <div>
              <a className="text-primary underline" href="/about">
                About RideLink
              </a>
            </div>
            <div>
              <a className="text-primary underline" href="/safety">
                Safety & security
              </a>
            </div>
            <div>
      ...
      <Card className="mt-6">
        ...
        <CardContent>
          <div className="text-sm text-muted-foreground space-y-2">
            ...
            <div>
              Documents are used for verification only; see Safety for details.
            </div>
